...
class DQNAgent:
    def __init__(self, state_size, action_size, learning_rate=0.001, gamma=0.99, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.01, replay_buffer_capacity=10000, batch_size=64, target_update_freq=100):
        ...
        self.target_q_network.load_state_dict(self.q_network.state_dict()) # Initialize target network
        self.target_q_network.eval() # Set to evaluation mode
    ...
    def learn(self):
        ...
        # We only want the Q-value for the action that was actually taken
        current_q_values = self.q_network(states_tensor).gather(1, actions_tensor.unsqueeze(1)).squeeze(1)

        # Get next Q-values from target network (max_a' Q_target(s', a'))
        with torch.no_grad(): # No gradient calculation for target network
            next_q_values_target = self.target_q_network(next_states_tensor).max(1)[0] # max(1)[0] gets max value along dim 1
            # If done, the next state has no future reward
